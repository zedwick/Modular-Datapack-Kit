...
#
# Sets game state to Lobby, and handles any other requirements.
#
...
scoreboard players operation game_state mdk_rounds_val = state_lobby mdk_rounds_const

# Setup (start) timer
scoreboard players operation time_remaining mdk_rounds_val = lobby_timer mdk_rounds_val

# Run functions tagged to run when state changes
function #mdk_rounds:event/game_state_changed
# Run functions tagged to run when state changes to lobby...
function #mdk_rounds:event/game_state_changed/to_lobby